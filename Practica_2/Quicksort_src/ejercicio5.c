...
int main(int argc, char** argv)
{
  ...
  if (argc != 13) {
    ...
    fprintf(stderr, "-retardo: numero de veces que se va a reordenar la tabla\n");
    exit(-1);
  }
  ...
  for(i = 1; i < argc ; i++) {
    ...
    } else if (strcmp(argv[i], "-retardo") == 0) {
      retardo = atoi(argv[++i]);
    } else {
  ...
  ret = genera_tiempos_ordenacion(quicksort_src, nombre,num_min, num_max,incr, n_perms, retardo);

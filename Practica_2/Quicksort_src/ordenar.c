...
int quicksort(int* tabla, int ip, int iu, pfunc_pivote pivote){
    ...
    /* Control de errores de los parametros de entrada */
    if((pivote==NULL)||(ip<0)||(iu<ip)|| (tabla == NULL)) 
            return ERR;
    ...
    if (ip<iu){
        ...
        /*Retorna el pivote y redistribuye los elementos*/
        if((m=partir(tabla, ip, iu, pivote))==ERR)
            return ERR;
        
        ob_count += iu-ip;
        
        /*si el primer indice es menor que el indice anterior
         al pivote*/
        if(ip<m-1){
            
            /*Recursion sobre la tabla izquierda*/
            if((ret=quicksort(tabla, ip, m-1, pivote))==ERR) 
                return ERR;
            
            ob_count+=ret;
        }
        /*si el indice siguiente al pivote es menor que el ultimo*/
        if(m+1<iu){
            
            /*Recursion sobre la tabla derecha*/
            if((ret=quicksort(tabla,m+1,iu,pivote))==ERR)
                return ERR;
...
int quicksort_src(int* tabla, int ip, int iu)
{
    ...
    /*mientras el primer indice es menor que el ultimo*/
    while ((iu-ip)>1){
        
        /*Retorna el pivote y redistribuye los elementos*/
        if((m=partir(tabla, ip, iu, medio))==ERR)
            return ERR;
